...
import me.marcdoesntexists.realms.enums.CasusBelli;
...
public class WarCommand implements CommandExecutor, TabCompleter {
    ...
    public WarCommand(Realms plugin) {
        this.plugin = plugin;
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        if (args.length == 1) {
            return me.marcdoesntexists.realms.utils.TabCompletionUtils.match(Arrays.asList("declare", "end", "info", "list", "score", "siege", "raid"), args[0]);
        }

        if (args.length == 2) {
            if (args[0].equalsIgnoreCase("declare") || args[0].equalsIgnoreCase("end") || args[0].equalsIgnoreCase("score") || args[0].equalsIgnoreCase("raid")) {
        ...
        if (args.length == 3 && args[0].equalsIgnoreCase("declare")) {
            return me.marcdoesntexists.realms.utils.TabCompletionUtils.matchDistinct(Arrays.stream(CasusBelli.values()).map(CasusBelli::name).toList(), args[2]);
        }
