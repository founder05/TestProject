...
public class MilitaryCommand implements CommandExecutor, TabCompleter {
    ...
    public MilitaryCommand(Realms plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        switch (subCommand) {
            case "enlist":
                ...
                if (args.length > 1 && !player.hasPermission("realms.military.enlist")) {
                    player.sendMessage(MessageUtils.get("military.no_permission_enlist"));
                    return true;
                }
                return handleEnlist(player, args);
            case "discharge":
                if (!player.hasPermission("realms.military.manage")) {
            ...
            case "info":
                if (!player.hasPermission("realms.military.view")) {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            return me.marcdoesntexists.realms.utils.TabCompletionUtils.match(Arrays.asList("enlist", "discharge", "promote", "demote", "info", "list", "ranks"), args[0]);
        }
        ...
        if (args.length == 3) {
            if (args[0].equalsIgnoreCase("promote") || args[0].equalsIgnoreCase("demote")) {
                return me.marcdoesntexists.realms.utils.TabCompletionUtils.match(new ArrayList<>(RANK_HIERARCHY.keySet()), args[2]);
            }
        }
