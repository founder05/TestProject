...
import me.marcdoesntexists.realms.societies.NobleTier;
...
public class NobleCommand implements CommandExecutor, TabCompleter {
    ...
    public NobleCommand(Realms plugin) {
        this.plugin = plugin;
    ...
    private boolean handleInfo(Player player, String[] args) {
        ...
        if (data == null) {
            player.sendMessage(MessageUtils.get("player_data_not_found"));
            return true;
        }

        NobleTier tier = data.getNobleTier();
        if (tier == null) tier = NobleTier.COMMONER; // fallback
    ...
    private boolean handlePromote(Player player, String[] args) {
        ...
        // permission check
        if (!player.hasPermission("realms.noble.promote")) {
        ...
        if (targetData == null || !Objects.equals(targetData.getTown(), data.getTown())) {
        ...
        if (promoterTier == null) promoterTier = NobleTier.COMMONER;
    ...
    private boolean handleDemote(Player player, String[] args) {
        ...
        if (currentTier == null) currentTier = NobleTier.COMMONER;
    ...
    private boolean handleList(Player player, String[] args) {
        ...
        for (UUID memberId : town.getMembers()) {
            ...
            NobleTier tier = memberData == null ? NobleTier.COMMONER : memberData.getNobleTier();
            if (tier == null) tier = NobleTier.COMMONER;
        ...
        for (NobleTier tier : NobleTier.values()) {
            ...
            if (!members.isEmpty()) {
                ...
                for (UUID memberId : members) {
                    ...
                    if (memberName == null) memberName = memberId.toString();
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        if (args.length == 1) {
            return me.marcdoesntexists.realms.utils.TabCompletionUtils.match(Arrays.asList("info", "promote", "demote", "list", "requirements", "upgrade"), args[0]);
        }
        ...
        if (args.length == 3 && args[0].equalsIgnoreCase("promote")) {
            return me.marcdoesntexists.realms.utils.TabCompletionUtils.match(Arrays.asList("KNIGHT", "BARON", "COUNT", "DUKE", "PRINCE", "KING"), args[2]);
        }

        if (args.length == 2 && args[0].equalsIgnoreCase("requirements")) {
            return me.marcdoesntexists.realms.utils.TabCompletionUtils.match(Arrays.asList("KNIGHT", "BARON", "COUNT", "DUKE", "PRINCE", "KING"), args[1]);
        }
