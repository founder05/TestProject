...
public class LawCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean handleReport(Player player, String[] args) {
        ...
        Crime crime = new Crime(criminal.getUniqueId(), crimeType, data.getTown(), location);
        lawManager.registerCrime(crime);
        
        player.sendMessage("§a✔ Crime reported successfully!");
        player.sendMessage("§7Criminal: §e" + criminal.getName());
        player.sendMessage("§7Crime: §c" + crimeType.getDisplayName());
        player.sendMessage("§7Fine: §6$" + crimeType.getBaseFine());
        player.sendMessage("§7Crime ID: §e" + crime.getCrimeId().toString().substring(0, 8));
